...
void cle_part(){	
	double p = 2.6/100;
	printf("On veut pour une certaine clé k que p(k) = %3f\n", p);
	byte_t k1 = 0, k2 = 0;
	...
	for(k1=0; k1<=0xf; k1++){
		for(k2=0; k2<=0xf; k2++){
			...
			for(i=0; i<5000; i++){
				...
				int r2 = read(fd,&c2,2);
				if(r1>0 && r2>0){
					//dechiffrement des couples
					c1^=k;
					c2^=k;
					c1 = heys_subst(c1, isbox);
					c2 = heys_subst(c2, isbox);
					block_t d = c1^c2;
					if(d==0x0606){nb++;}
				}
	...
	for(b1=0; b1<=0xff; b1++){
		for(b2=0; b2<=0xff; b2++){
			for(b3=0; b3<=0xff; b3++){
				for(b4=0; b4<=0xf; b4++){
					for(b5=0; b5<=0xf; b5++){	
						ck = (b1<<32)^(b2<<24)^(b3<<16)^(k)^(b4<<8)^(b5);
						int i; int egal = 1;
						lseek(fd,0,SEEK_SET);
						lseek(fp,0,SEEK_SET);
						for(i=0; i<10000; i++){
